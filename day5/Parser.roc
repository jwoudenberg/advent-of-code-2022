interface Parser
    exposes [Parser, ParseError, run, str, any, map, with, many, byte, success, try, nat, line]
    imports []
...
line =
  @Parser (\bytes ->
    when List.splitFirst bytes 10 is
      Err _ -> Ok { result: bytes, remaining: [] }
      Ok { before, after } -> Ok { result: before, remaining: List.dropFirst after }
  )
